...
class FreetypeRecipe(Recipe):
    ...
    def get_recipe_env(self, arch=None, with_harfbuzz=False):
        ...
        if with_harfbuzz:
            ...
            freetype_install = join(
                self.get_build_dir(arch.arch), 'install'
            )

            env['HARFBUZZ_CFLAGS'] = (
                '-I{harfbuzz} -I{harfbuzz}/src'.format(
        ...
        add_flag_if_not_added(
            ' -I' + zlib_includes, 'CFLAGS'
        )
        add_flag_if_not_added(
            ' -L' + zlib_lib_path, 'LDFLAGS'
        )
        add_flag_if_not_added(
            ' -lz', 'LDLIBS'
        )
    ...
    def build_arch(self, arch, with_harfbuzz=False):
        env = self.get_recipe_env(
            arch, with_harfbuzz=with_harfbuzz
        )

        harfbuzz_in_recipes = (
            'harfbuzz' in self.ctx.recipe_build_order
        )
        ...
        if not harfbuzz_in_recipes:
            ...
            info(
                'Build freetype for Second time '
                '(with harfbuzz)'
            )
        ...
        with current_directory(
            ...
            if not with_harfbuzz and harfbuzz_in_recipes:
                info(
                    'Installing freetype '
                    '(first time build without harfbuzz)'
                )
    ...
    def install_libraries(self, arch):
        if not exists(
            list(self.get_libraries(arch))[0]
        ):
            return

        self.install_libs(
            arch,
            *self.get_libraries(arch)
        )
